...
The portable C implementation currently supports the same electrical
configuration validation and threshold-register encoding rules as Python. The
shared threshold vectors run against both languages. Native register decoding
and the SPI/delay-driven one-shot acquisition sequence are still under 0.2
development, so the C MAX31865 driver is not yet complete.
